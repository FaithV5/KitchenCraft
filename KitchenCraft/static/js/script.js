...
async function saveMenu(menu) {
    ...
    // Persist to sessionStorage so changes reset when the browser is closed
    try {
        sessionStorage.setItem('kitchencraft_menu', JSON.stringify(menu));
        ok = true;
        console.log('Menu saved to sessionStorage');
    } catch (err) {
        console.error('Failed to save menu to sessionStorage', err);
    }
...
async function addToCart(itemName, itemPrice, quantity = 1) {
    const currentUser = getCurrentUser();
    if (!currentUser) {
        // Inform the user they must login to order and do not redirect automatically
...
// Resolve template-relative path: if current page is inside /templates/, return the filename
// otherwise prefix with 'templates/' so links point to the templates folder from root.
function resolveTemplatePath(filename) {
    try {
        // Always return an absolute path so redirects work correctly from any page.
        // Special-case the site root `index.html` to be at `/index.html`.
...
// Category display helper (centralized so templates don't need to embed mappings)
function humanizeCategory(key) {
    const k = (key || '').toString().toLowerCase();
    return (key || 'uncategorized').toString().split(/[-_\s]+/).map(part => part.charAt(0).toUpperCase() + part.slice(1)).join(' ');
}
...
function updateAuthLinks() {
    ...
    const headerLogoutBtn = document.getElementById('header-logout');
        ...
        const loginHref = resolveTemplatePath('login.html');
        authLinks.innerHTML = `
...
function ensureRoleNavLinks(role) {
    ...
    if (role === 'admin') {
        // Use template-aware paths
        add(resolveTemplatePath('home.html'), 'Home');
        add(resolveTemplatePath('adminmenu.html'), 'Manage Products');
        add(resolveTemplatePath('manage_users.html'), 'Manage Users');
        add(resolveTemplatePath('admin_orders.html'), 'Orders');
        add(resolveTemplatePath('analytics.html'), 'Analytics');
        return;
    }

    if (role === 'rider') {
        add(resolveTemplatePath('home.html'), 'Home');
        add(resolveTemplatePath('rider_dashboard.html'), 'Rider Dashboard');
        add(resolveTemplatePath('contact.html'), 'Contact');
        return;
    }

    // Not logged in: show only browsing links
    if (!role) {
        add(resolveTemplatePath('home.html'), 'Home');
        add(resolveTemplatePath('products.html'), 'Products');
        add(resolveTemplatePath('contact.html'), 'Contact');
        return;
    }

    // Logged-in customer navigation (show cart and status)
    add(resolveTemplatePath('home.html'), 'Home');
    add(resolveTemplatePath('products.html'), 'Products');
    add(resolveTemplatePath('cart.html'), 'Cart (<span id="cart-count">0</span>)');
    add(resolveTemplatePath('status.html'), 'Delivery Status');
    add(resolveTemplatePath('contact.html'), 'Contact');
}
...
function checkAuthentication() {
    ...
    if (currentPage === 'admin.html' || currentPage === 'adminmenu.html') {
            ...
            setTimeout(() => { try { window.location.href = resolveTemplatePath('login.html'); } catch (e) { window.location.href = 'templates/login.html'; } }, 1500);
            return false;
        }
    }
...
async function updateOrderStatus(orderId, newStatus) {
    ...
    // Prefer backend store helper which handles stock adjustments centrally
    if (window.store && window.store.updateOrderStatus) {
        try {
            await window.store.updateOrderStatus(orderId, newStatus);
            showNotification('Order updated', 'success');
            return true;
        } catch (e) {
            console.warn('window.store.updateOrderStatus failed, falling back to local update', e);
            // fall through to local handling
        }
    }

    // Local fallback: update status and, if cancelling, restore stock
    ...
    if (String(newStatus).toLowerCase() === 'cancelled' && String(prevStatus).toLowerCase() !== 'cancelled') {
        try {
            ...
            console.warn('Failed to locally restore stock on cancellation', e);
        }
    }
...
async function placeOrder() {
    ...
    // Check inventory levels before placing order
    try {
        const menu = await getMenuItems();
        for (const si of selectedItems) {
            const menuIdx = menu.findIndex(mi => si.name === mi.name || si.name.startsWith(mi.name + ' ') || si.name.startsWith(mi.name + '('));
            if (menuIdx !== -1) {
                const mi = menu[menuIdx];
                if (typeof mi.stock === 'number' && mi.stock < (si.quantity || 1)) {
                    alert(`Not enough stock for ${si.name}. Available: ${mi.stock}`);
                    return;
                }
            }
        }
    } catch (e) {
        console.warn('Inventory check failed, proceeding with order', e);
    }
    ...
    // Prefer backend store helper (which also decrements stock) when available
    try {
        if (window.addOrder) {
            await window.addOrder(newOrder);
        } else {
        ...
        console.warn('window.addOrder failed, falling back to saveOrders', e);
        orders.push(newOrder);
        await saveOrdersToStorage(orders);
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    try {
        if (!sessionStorage.getItem('kitchencraft_session_started')) {
            ...
            // Prefer the backend-provided reset helper when available — but only run
            // when explicitly requested by developers. Automatic resets on every new
            // session can overwrite runtime changes (orders/users) made in other
            // tabs; gate this behind a flag to avoid unexpected data loss.
            try {
                const forceReset = localStorage.getItem('kitchencraft_force_reset') === 'true';
                if (forceReset && window.resetToDefaults) {
                    console.log('kitchencraft_force_reset=true — resetting store to defaults');
                    try { window.resetToDefaults(); } catch (e) { console.warn('resetToDefaults error', e); }
                }
            } catch (e) { console.warn('No resetToDefaults available or flag check failed', e); }
